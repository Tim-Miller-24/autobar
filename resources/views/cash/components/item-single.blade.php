        ...
        <div class="flex justify-between items-center p-2">
            ...
            <div class="inline-flex right">
                @if($item->options->count())
                    ...
                    <a href="#option_{{ $item->id }}" rel="modal:open" class="uppercase focus:outline-none bg-purple-800 hover:bg-indigo-600 text-white font-bold py-2 px-3 shadow inline-flex items-center">
                ...
                @else
                    @if(is_array($cart_items) AND array_key_exists($item->id, $cart_items))
                        <button wire:click="remove({{ $item->id }}, '1')" class="focus:outline-none bg-red-600 hover:bg-red-800 text-white font-bold py-2 px-3">
                        ...
                        @if($cart_items[$item->id]['quantity'] < $item->stock)
                            <button wire:click="add({{ $item->id }})" class="focus:outline-none bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-3">
                                @else
                                    ...
                                    <button wire:click="remove({{ $item->id }})" class="focus:outline-none ml-2 bg-red-600 hover:bg-red-800 text-white font-bold py-2 px-3 shadow inline-flex items-center">
                                    ...
                                    @else
                                        <button wire:click="add({{ $item->id }})" class="focus:outline-none bg-purple-800 hover:bg-indigo-600 text-white font-bold py-2 px-3 shadow inline-flex items-center">
                                            <svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clip-rule="evenodd" />
                                            </svg>
