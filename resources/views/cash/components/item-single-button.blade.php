@if(is_array($cart_items) AND array_key_exists($item->id, $cart_items))
    <div class="float-right">
        <button wire:click="remove({{ $item->id }}, '1')" class="focus:outline-none bg-red-800 text-white text-xl text-white font-bold p-3 inline-block">
        ...
        <div class="text-primary font-bold px-2 relative bottom-1 inline-block">{{ $cart_items[$item->id]['quantity'] }}</div>
        @if($cart_items[$item->id]['quantity'] < $item->stock)
            <button wire:click="add({{ $item->id }})" class="focus:outline-none bg-green-800 text-white text-xl text-link-main font-bold p-3 inline-block">
        @else
    ...
    @else
        <button wire:click="add({{ $item->id }})"
                class="focus:outline-none bg-active text-xl text-white p-2 w-full shadow-inner inline-block">
