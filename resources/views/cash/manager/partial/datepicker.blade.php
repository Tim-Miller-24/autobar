<div x-data="{{ $pickerId }}()" x-init="[initDate(), getNoOfDays()]">
    <label for="datepicker" class="font-bold mb-1 text-gray-700 block">{{ $text }}</label>
    <div class="relative">
        <input type="hidden" name="{{ $pickerId }}" x-ref="date" :value="datepickerValue">
        <input type="text" readonly x-model="datepickerValue" @click="showDatepicker = !showDatepicker" @keydown.escape="showDatepicker = false" class="w-full pl-4 pr-10 py-3 leading-none rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium" placeholder="Выбрать число">
        ...
        function {{ $pickerId }}() {
            return {
                ...
                getDateValue(date) {
                    ...
                    this.datepickerValue = formatDate(selectedDate);
                    this.$refs.date.value = formatDate(selectedDate);
