            ...
            <div class="col-span-6">
                ...
                AND (isset($cart_items[$item->id]['options']) AND array_key_exists($option->id, $cart_items[$item->id]['options'])))
                    <div class="float-right">
                        <button wire:click="remove({{ $item->id }}, 1, {{ $option->id }})" class="focus:outline-none bg-red-800 text-white text-xl text-white font-bold p-3">
                        ...
                        <span class="text-primary font-bold px-2 relative bottom-1">
                        ...
                        @if($cart_items[$item->id]['options'][$option->id]['quantity'] < $item->getStockOption($option->id))
                            <button wire:click="add({{ $item->id }}, 1, {{ $option->id }})" class="focus:outline-none bg-green-800 text-white text-xl text-link-main font-bold p-3">
                        @else
